...
export type { UserAttentionReason } from "./types";
...
export function getUserAttentionReasons(
  ...
  return [...user.attention_reasons];
}

export function deriveUserAttentionReasons(
  user: MobileUserHealthRow
): UserAttentionReason[] {
  const reasons: UserAttentionReason[] = [];
  if (user.auth.state !== "ready") reasons.push("auth_blocked");

  if (user.data.setup_mode === "seeded") {
