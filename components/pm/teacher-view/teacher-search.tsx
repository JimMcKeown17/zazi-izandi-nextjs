...
interface Props {
  eas: TeacherSearchItem[];
}

export function TeacherSearch({ eas }: Props) {
  ...
  const filtered = useMemo(() => {
    ...
    return eas.filter(
      (ea) =>
        ...
        ea.grade.toLowerCase().includes(q)
    );
  }, [query, eas]);
    ...
    <div className="space-y-3">
      <div className="relative">
        ...
        <input
          ...
          placeholder="Search by EA name, school, or grade…"
      ...
      <p className="text-xs text-slate-500">
        {filtered.length}{" "}
        {filtered.length === 1 ? "classroom matches" : "classrooms match"} your
        search
      </p>
      ...
      <ul className="divide-y divide-slate-100 rounded-xl border border-slate-200 bg-white">
        {filtered.length === 0 ? (
          <li className="px-4 py-6 text-center text-sm text-slate-400">
            No classrooms match &ldquo;{query}&rdquo;
          </li>
        ) : (
          filtered.map((ea) => (
            <li key={`${ea.ea_user_id}-${ea.ea_name}`}>
              <Link
                href={`/pm/teacher-view/${ea.ea_user_id}`}
              ...
              >
                <div className="min-w-0">
                  ...
                  <p className="truncate text-xs text-slate-500">
                    {ea.school} · {ea.grade} · {ea.children_count} children
                  </p>
                </div>
