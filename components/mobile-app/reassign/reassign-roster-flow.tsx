...
function compactJobProgress(job: MobileHandoverJobResponse): string {
  return `${job.job.status}:${job.job.progress_cursor}:${job.items.map((item) => item.state).join(",")}`;
}

function isTerminal(job: MobileHandoverJobResponse): boolean {
...
export function MobileReassignRosterFlow({ candidates }: { candidates: MobileReassignEaOption[] }) {
  ...
  async function runContinuations(initial: MobileHandoverJobResponse) {
    let current = initial;
    let previousProgress = compactJobProgress(initial);
    setBusy(true);
    setError(null);
    while (!isTerminal(current) && current.job.retryable) {
      const result = await executeMobileReassignment(current.job.id);
      if (!result.ok) {
      ...
      current = result.data;
      setJob(current);
      const nextProgress = compactJobProgress(current);
      if (isTerminal(current) || !current.job.retryable || nextProgress === previousProgress) break;
    ...
    <div data-testid="mobile-reassign-roster-flow" className="mx-auto max-w-6xl space-y-5">
      ...
      <section className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
        ...
        <div className="mt-4 flex flex-wrap gap-2">
          <Button type="button" onClick={() => void loadPreview()} disabled={busy || !fromEa}>{busy ? "Loading…" : "Preview roster"}</Button>
      ...
      {preview ? <>
        ...
        <section className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
          ...
          <div className="mt-3 flex items-center justify-between gap-3"><p className="text-xs text-slate-500">{reason.trim().length}/200 characters</p><Button type="button" onClick={() => setConfirmOpen(true)} disabled={!toEa || !reason.trim() || !allUnresolvedDecided || busy}>Review and confirm</Button></div>
        </section>
      </> : null}
