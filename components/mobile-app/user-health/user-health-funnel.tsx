...
interface UserHealthFunnelProps {
  counts: FunnelCounts;
  days: number;
}

export function UserHealthFunnel({
  counts,
  days,
}: UserHealthFunnelProps) {
  const barRows = [
    {
      label: "Accounts",
    ...
    <section className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm sm:p-5">
      <div>
        <h2 className="text-base font-bold text-slate-900">
          Onboarding evidence coverage
        </h2>
        <p className="mt-1 text-xs leading-relaxed text-slate-500">
          Each row is independent evidence over all accounts — rows are not
          subsets of each other (an EA can be active without a device signal).
        </p>
      </div>
