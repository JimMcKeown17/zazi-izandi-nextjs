...
**Page (`page.tsx`):** extend the `PREDICATES` parse list with `"activated"` and `"quiet"` (accepted only when `hasPartBCapability(data)`; otherwise they parse to `"all"`); parse the `wave` param — `parseWave(value, waveOptionIds)` returns `"all"` when absent/unknown, `"none"`, or a validated wave id; compute `const lifetimeEvidence = hasPartBCapability(data);` ONCE and pass it to BOTH `UserHealthBoard` and `HowToReadPanel` (round-9 finding); pass `waveOptions={data.wave_options ?? []}` and `initialWave` to `UserHealthBoard`; add the wave value and the capability boolean to the board remount key string.
...
**How-to panel (`how-to-read-panel.tsx`):** the panel gains a `lifetimeEvidence: boolean` prop and the page passes the SAME `hasPartBCapability(data)` value to it as to the board (round-9 finding — every explanatory surface degrades together, or the panel keeps teaching semantics the board just hid). When `false`: keep the CURRENT Part A copy unchanged (window/current-device explanations; no `Activated`, `Quiet`, wave, or app-open text anywhere). When `true`: stages are durable — `Activated` means the EA has EVER produced app activity (it can never go backwards; shrinking the window cannot demote anyone); `Reached` includes devices whose push token later died (`ever_registered_device`) AND accounts whose only proof is a signed-in app open; windowed claims live in the separate indicators — `Active · {days}d` (usage in the selected window, the same number the summary tile counts) and `Quiet · {days}d` (activated-ever but silent in the window). State plainly that the `Active` filter and tile are windowed while the stage badge is lifetime — they answer different questions. Add a wave paragraph: the wave filter scopes the board and the evidence strip to one rollout wave; `day n` counts whole days since launch in SAST; "No wave" shows accounts not assigned to any wave. Add an app_open note with the reach-not-usage framing verbatim: `An "Opened" date is proof the app was opened by this signed-in account — it says the app reached them, not that they are teaching with it or still using it. Older app versions don't send it, so a missing date is not proof of absence.`
...
const AppOpenReporter = () => {
  ...
  const { isOnline } = useOffline(); // OfflineContext exposes the scheduler's isOnline
...
**Connectivity retry (round-8 finding):** a cold launch can pass the local session check, fail the RPC offline, and then regain connectivity while still foregrounded — with no AppState event and no session change, nothing would retry. `OfflineContext` sits ABOVE `AuthProvider` in the provider order and its `useOffline()` hook exposes `isOnline` (verified against live source in review round 9) — the effect re-runs on the offline→online transition because `isOnline` is a dependency, and the service's per-user recorded/in-flight guards make the extra invocations no-ops after success. Verify the destructured name against `src/context/OfflineContext.js` when implementing; only if it has genuinely moved, fall back to a bounded interval INSIDE the effect (retry every 60s while unrecorded, max 5 attempts, cleared on unmount and on success) instead of adding new context surface.
